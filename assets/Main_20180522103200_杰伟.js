...
function flyball() {
    ...
    var interval2 = window.setInterval((function () {
        // createBall(stage[2], centerY - 210, true);  // scale
        var speedx = 7 - (Math.random() * 14) * 70 * (ctn % 2 == 0 ? 1 : 0);
        var speedy = -300;
        createBall1(centerX + Math.random() * 10, scale * 760, speedx, speedy); // scale
        ctn += 1
    }), inter);
    setTimeout(function () {
        window.clearInterval(interval2);
...
function createBall1(x, y, speedx, speedy) {
    var size = scale * 50 * 2;
    ...
    b2body.linearVelocity.Set(speedx, speedy);
    bodies1.push(world1.CreateBody(b2body));
    // world.CreateBody(b2body)
}
...
function step(cnt) {
    ...
    world1.m_gravity.x = gravity.x * 350 + delta[0];
    world1.m_gravity.y = gravity.y * 350 + delta[1];
